  ...
  app <- shinyApp(
    ui = fluidPage(
      ...
      tabsetPanel(
        tabPanel(
          ...
          column(
            ...
            sliderInput("rt", "X-Axis Limits (Retention Time)",
              min = floor(min(peakdata$peakgroup_rt)),
              max = ceiling(max(peakdata$peakgroup_rt)),
              value = c(floor(min(peakdata$peakgroup_rt)), ceiling(max(peakdata$peakgroup_rt))),
              step = 10, round = 1
            ),
            checkboxInput("y_auto", "Auto Y-Axis", value = TRUE),
            sliderInput("mz", "Y-Axis Limits (m/z)",
              min = floor(min(peakdata$LOBdbase_mz)),
              max = ceiling(max(peakdata$LOBdbase_mz)),
              value = c(floor(min(peakdata$LOBdbase_mz)), ceiling(max(peakdata$LOBdbase_mz))),
              step = 10, round = 1
            ),
          ...
          column(
            3,
            selectInput("class", "Select Lipid Class", c("All", as.character(unique(peakdata$species))), multiple = TRUE, selected = "All"),
            ...
            selectInput("total_carbon", "Acyl Carbon", c("All", unique(peakdata$FA_total_no_C)), multiple = TRUE, selected = "All"),
            selectInput("total_db", "Doubled Bonds", c("All", unique(peakdata$FA_total_no_DB)), multiple = TRUE, selected = "All"),
            selectInput("sizebysample", "Size by Sample", c("None", LOBset@sampnames), multiple = FALSE, selected = "None"),
        ...
        tabPanel(
          ...
          column(
            4,
            selectInput("species", "Select Species", c("All", as.character(unique(peakdata$species))), multiple = FALSE),
          ...
          column(
            4,
            selectInput("flag", "Flag?", c("All", as.character(unique(peakdata$Flag))), multiple = TRUE, selected = "All")
          )
        )
      )
    ),
    ...
    server = function(input, output) {
      ...
      output$plot <- renderPlot({
        data <- peakdata # so we dont change our intial data
        ...
        output$info <- renderTable({
          ...
          run_table <- nearPoints(data, input$plot_click,
            threshold = 20, maxpoints = 1,
            addDist = TRUE
          )
          columns <- c("xcms_peakgroup", "compound_name", "LOBdbase_mz", "peakgroup_rt", "Flag", "lpSolve", "code")
          peakdata[which(peakdata$xcms_peakgroup == run_table$xcms_peakgroup), columns[which(columns %in% colnames(peakdata))]]
        }, digits = 5)
        plot(g)
      })
    }
