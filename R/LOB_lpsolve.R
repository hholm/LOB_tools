...
for (k in 1:length(unique(LOBpeaklist$species))) {
  ...
  screened_plot <- screened[which(is.na(screened$lpSolve)==FALSE & ms2v_screened$species == unique(LOBpeaklist$species)[k]),]
  ...
  if (use_ms2==TRUE & Final_Switch == TRUE) {
    ...
    if("No" %in% test){
      cat("\n")
      cat("ERROR: Not all MS2v peaks included in solution.")
